...
// ID identifie une facture ou un acompte. C'est un UUID version 4 dans sa forme
// canonique.
//
// Le type est distinct de string, et distinct des identifiants des autres
// domaines (devis.ID, document.ID…) : le compilateur refuse ainsi de confondre
// l'identifiant d'une facture avec celui d'un devis, ce qui est l'objet de R2 de
...
//
// L'aléa vient de crypto/rand : un identifiant de facture finit dans des URLs,
// et un générateur prévisible ferait du registre des dépenses une information
...
func NewID() (ID, error) {
	...
	if _, err := rand.Read(raw[:]); err != nil {
		return "", fmt.Errorf("tirage d'un identifiant de finance : %w", err)
	}
...
// ActeurID désigne la personne qui a signé une action — saisi une facture,
// enregistré un acompte, marqué un remboursement.
//
