...
// ID identifie une étape ou un jalon. C'est un UUID version 4 dans sa forme
// canonique.
//
...
// l'identifiant d'une étape avec celui d'un devis, ce qui est l'objet de R2 de
...
//
// L'aléa vient de crypto/rand : un identifiant d'étape finit dans des URLs, et
// un générateur prévisible ferait du planning du chantier une information
// devinable. L'erreur est propagée plutôt qu'ignorée — un crypto/rand en panne
// est une raison de refuser d'enregistrer, pas de continuer avec un
// identifiant douteux.
func NewID() (ID, error) {
	var raw [16]byte
	if _, err := rand.Read(raw[:]); err != nil {
		return "", fmt.Errorf("tirage d'un identifiant de planning : %w", err)
	}
...
// ActeurID désigne la personne qui a signé une action — créé une étape,
// démarré un lot, atteint un jalon.
//
